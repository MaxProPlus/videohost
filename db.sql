...
CREATE TRIGGER estimate_create
    AFTER INSERT
    ON videohost.estimate
    FOR EACH ROW
BEGIN
    UPDATE videohost.video SET rating = rating + 1 WHERE id = NEW.id_video;
    case when NEW.star = 1 then UPDATE videohost.video SET `like` = `like` + 1 WHERE id = NEW.id_video;
        WHEN NEW.star = 2 then UPDATE videohost.video SET dislike = dislike + 1 WHERE id = NEW.id_video;
        else begin end;
        end case;
end;
    ...
    case when OLD.star = 0 then
        ...
        when OLD.star = 1 then
            case when NEW.star = 1 then begin
                UPDATE videohost.video SET `like` = `like` - 1 WHERE id = NEW.id_video;
                SET NEW.star = 0;
            end;
                WHEN NEW.star = 2 then begin
                    UPDATE videohost.video SET dislike = dislike + 1 WHERE id = NEW.id_video;
                    UPDATE videohost.video SET `like` = `like` - 1 WHERE id = NEW.id_video;
                end; else begin end;
                end case;
